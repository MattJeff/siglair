...
| `EMAIL_API_BASE` | `https://api.resend.com` | l'origine à qui l'adaptateur Resend parle. **Refusée au démarrage sans `AGENTOS_ALLOW_MOCKS=1`**, parce qu'elle décide à qui `EMAIL_API_KEY` est présentée. N'est pas un faux et n'entre pas dans `mock_adapters` — le boot dit toujours `email=resend`. Sa seule raison d'être est `scripts/faux-resend.py`, la répétition de `--reel` ; sur un déploiement, ne pas la poser |
