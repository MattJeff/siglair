...
| `AGENTOS_APPROVAL_NOTIFY` | unset | **No mail leaves when an approval enters the queue**; it waits on `approvals_list` for up to 24 h ([§7](#7-approvals)), and the boot says so at INFO. Set it to the founder's address and every approval filed with a draft — a seat's reply to a stranger held for review — is mailed there in full: the letter, the approval id, and the two gestures (`approvals_approve` / `approvals_deny`, with the **approver** key). Sender is the asking seat's address on the tenant's verified primary domain; idempotent per approval, so an outbox replay does not write twice. No policy rules on this mail — it is the platform writing to its operator, not a seat writing to a stranger. A value that is not an email address is a boot failure naming the variable. |
...
| `EMAIL_API_BASE` | `https://api.resend.com` | Where that real client talks. **Refused at boot unless `AGENTOS_ALLOW_MOCKS=1`**, because it decides who `EMAIL_API_KEY` is presented to — a redirected adapter is not a mock, it is real and talking to somebody else, so it is barred where mocks are barred rather than listed beside them. The boot line still reads `email=resend`. It exists for `scripts/faux-resend.py`, the `--reel` rehearsal; never set it on a deployment. |
